...
function parseArguments(arguments_) {
  if (!Array.isArray(arguments_) || arguments_.length !== 4 && arguments_.length !== 12) fail();
  const values = Object.create(null);
  for (let index = 0; index < arguments_.length; index += 2) {
    const key = arguments_[index];
    const value = arguments_[index + 1];
    if (![...BASE_OPTIONS, ...PAYER_MCP_OPTIONS].includes(key) || typeof value !== "string" || value.length === 0 || Object.hasOwn(values, key)) fail();
    values[key] = value;
  }
